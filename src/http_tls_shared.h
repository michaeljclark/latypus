...
struct http_tls_session
{
    std::string sess_key;
    time_t sess_time;
    const unsigned char *sess_der;
    size_t sess_der_len;
    
    http_tls_session()
        : sess_key(), sess_time(0), sess_der(nullptr), sess_der_len(0) {}
    
    http_tls_session(std::string sess_key, time_t sess_time, const unsigned char *sess_der, size_t sess_der_len)
        ...
        : sess_key(o.sess_key), sess_time(o.sess_time), sess_der(nullptr), sess_der_len(o.sess_der_len)
    {
        if (o.sess_der && o.sess_der_len) {
            unsigned char *sess_der_tmp = new unsigned char[sess_der_len];
            memcpy(sess_der_tmp, o.sess_der, sess_der_len);
            sess_der = (const unsigned char *)sess_der_tmp;
        }
    }
...
struct http_tls_shared
{
    ...
    static SSL_SESSION * tls_get_session_cb(SSL *ssl, const unsigned char *data, int len, int *copy);
